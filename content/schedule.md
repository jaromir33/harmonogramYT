---
title: Harmonogram parzenia kawy
---

<div class="j33-description">
  <p>☕ <strong>Wieczna Pauza:</strong> Nowe odcinki zazwyczaj pojawiają się o 16:00.</p>
  <p>🔴 <strong>Na żywo:</strong> Transmisje LIVE ogłaszam w zakładce Społeczność – tam kawa smakuje najlepiej wspólnie.</p>
  <p style="font-size: 0.85em; opacity: 0.8;"><em>Pamiętaj: Plan to tylko zarys. Czasem taktyka wymaga korekty w trakcie tury.</em></p>
</div>

<table class="schedule">
<thead>
  <tr><th>Data</th><th>Seria</th><th>Odcinek</th><th>Typ</th></tr>
</thead>
<tbody>

<tr class="type-film">
  <td class="date-cell">11.05 pon.</td>
  <td>XCOM 2 WOTC + mody [PL]</td>
  <td>XCOM 2 WOTC + mody [PL], cz.61 - bio ekipa.</td>
  <td class="status-cell">🎬 Film</td>
</tr>
<tr class="type-film">
  <td class="date-cell">12.05 wt.</td>
  <td>Shadow Empire: Republica DLC</td>
  <td>Shadow Empire: Republica DLC, cz.13 - fajne auto i atak ze strony głównego reżimu.</td>
  <td class="status-cell">🎬 Film</td>
</tr>
<tr class="type-rzutoka">
  <td class="date-cell">13.05 śr.</td>
  <td>Oxygen Not Included</td>
  <td>I biegu przyspiesza, i gna coraz prędzej - Oxygen Not Included, cz.1 (PL)</td>
  <td class="status-cell">👁️ Rzut oka</td>
</tr>
<tr class="type-film">
  <td class="date-cell">14.05 czw.</td>
  <td>Shadow Empire: Republica DLC</td>
  <td>Shadow Empire: Republica DLC, cz.14 - potwory i pająki.</td>
  <td class="status-cell">🎬 Film</td>
</tr>
<tr class="type-film">
  <td class="date-cell">15.05 pt.</td>
  <td>Songs of Syx (PL)</td>
  <td>Demokratyczne ruchy. cz.22 - Songs of Syx (PL), Garthimis i Amevias.</td>
  <td class="status-cell">🎬 Film</td>
</tr>
<tr class="type-film">
  <td class="date-cell">16.05 sob.</td>
  <td>Oxygen Not Included</td>
  <td>A dokąd? A dokąd? A dokąd? Na wprost! NA NAS - Oxygen Not Included, cz.2 (PL)</td>
  <td class="status-cell">🎬 Film</td>
</tr>
<tr class="type-film">
  <td class="date-cell">17.05 niedz.</td>
  <td>Songs of Syx (PL)</td>
  <td>Pierwsi turyści. cz.23 - Songs of Syx (PL), Garthimis i Amevias.</td>
  <td class="status-cell">🎬 Film</td>
</tr></tbody></table>