---
title: Harmonogram
tags: []
---

<div class="j33-description">
  <p>☕ <strong>Nowe odcinki</strong> zazwyczaj pojawiają się o 16:00.</p>
  <p>🔴 <strong>Na żywo:</strong> Transmisje LIVE ogłaszam w zakładce Społeczność.</p>
  <p style="font-size:0.85em;opacity:0.7;"><em>Plan to tylko zarys — czasem taktyka wymaga korekty w trakcie tury.</em></p>
  <p style="font-size:0.8em;opacity:0.5;">Ostatnia aktualizacja: 19.09.2026</p>
</div>

<table class="schedule">
<thead>
  <tr><th>Data</th><th>Seria</th><th>Odcinek</th><th>Typ</th></tr>
</thead>
<tbody>
<tr class="type-rzutoka">
  <td class="date-cell">18.09 pt.</td>
  <td>ENDLESS Legend 2</td>
  <td>Architekci pustyni - cz.1 | ENDLESS Legend 2</td>
  <td class="status-cell">👁️ Rzut oka</td>
</tr>
<tr class="type-rzutoka">
  <td class="date-cell">19.09 sob.</td>
  <td>Scrap Mechanic</td>
  <td>Porządek w bazie i dostarczamy towar - Scrap Mechanic [PL], #9</td>
  <td class="status-cell">👁️ Rzut oka</td>
</tr>
<tr class="type-film">
  <td class="date-cell">20.09 niedz.</td>
  <td>ENDLESS Legend 2</td>
  <td>Niebezpieczna sytuacja - cz.2 | ENDLESS Legend 2</td>
  <td class="status-cell">🎬 Seria</td>
</tr>
<tr class="type-film">
  <td class="date-cell">21.09 pon.</td>
  <td>XCOM 2 WOTC + mody [PL]</td>
  <td>XCOM 2 WOTC + mody [PL], cz.80 - nie drzyj paszczy.</td>
  <td class="status-cell">🎬 Seria</td>
</tr>
<tr class="type-film">
  <td class="date-cell">22.09 wt.</td>
  <td>Star Wars: Zero Company</td>
  <td>Zaczynamy od zera w grze: Star Wars: Zero Company #1 [PL]</td>
  <td class="status-cell">🎬 Seria</td>
</tr>
<tr class="type-rzutoka">
  <td class="date-cell">23.09 śr.</td>
  <td>Scrap Mechanic</td>
  <td>Projekt "duże koła" - Scrap Mechanic [PL], #10</td>
  <td class="status-cell">👁️ Rzut oka</td>
</tr>
<tr class="type-film">
  <td class="date-cell">24.09 czw.</td>
  <td>Oxygen Not Included</td>
  <td>Skąd wziąć dobrą wodę i co jest z tymi maskami? | Oxygen Not Included: Aquatic Planet Pack, cz.13</td>
  <td class="status-cell">🎬 Seria</td>
</tr>
<tr class="type-rzutoka">
  <td class="date-cell">25.09 pt.</td>
  <td>ENDLESS Legend 2</td>
  <td>(jak zdążę nagrać to ENDLESS Legend 2)</td>
  <td class="status-cell">👁️ Rzut oka</td>
</tr>
<tr class="type-film">
  <td class="date-cell">26.09 sob.</td>
  <td>Star Wars: Zero Company</td>
  <td>Opcje taktyczne - "wal po ryju": Star Wars: Zero Company #2 [PL]</td>
  <td class="status-cell">🎬 Seria</td>
</tr>
<tr class="type-przerwa">
  <td class="date-cell">27.09 niedz.</td>
  <td></td>
  <td></td>
  <td class="status-cell">🏖️ Przerwa</td>
</tr>
</tbody></table>
